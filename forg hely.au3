...
if $a = 1 Then
	ClipPut($Temp1)
	MsgBox(0,"Probléma az alábbi jármuvekkel",$Temp & @LF&@LF&@LF & "A Típus megnevezés helytelen"&@LF&"Javítani kell az Automaster-ben"&@LF&"A JOB számok a vágólapra lettek másolva")
EndIf
;~ #EndRegion
...
$Form1 = GUICreate("Havi tervezo", 1281, 901, 225, 51)
$List1 = GUICtrlCreateListView("JOB   | Eseménykód | Típus | Vevo neve | Értékesíto | Kategória | Állapot | T - dátum | Megjegyzés",10,30,1000,400)
...
$List2 = GUICtrlCreateListView("JOB   | Rendszám | Típus | Vevo neve | Értékesíto | Kategória | Állapot | El. forg. hely. ",10,450,1000,400)
   GUICtrlSendMsg(-1, $LVM_SETCOLUMNWIDTH, 0, 80)
   GUICtrlSendMsg(-1, $LVM_SETCOLUMNWIDTH, 1, 40)
   GUICtrlSendMsg(-1, $LVM_SETCOLUMNWIDTH, 2, 50)
   GUICtrlSendMsg(-1, $LVM_SETCOLUMNWIDTH, 3, 50)
...
$tipus = GUICtrlCreateCombo("",1136, 40, 120, 25)
   ...
   _GUICtrlComboBox_AddString($tipus,"H=Haszongépjármu")
...
$ertek = GUICtrlCreateCombo("",1136, 100, 120, 25)
   ...
   _GUICtrlComboBox_AddString($ertek,"Lorincz László")
...
$Label1 = GUICtrlCreateLabel("Jármu típus", 1026, 40, 91, 24)
GUICtrlSetFont(-1, 12, 400, 0, "MS Sans Serif")
$Label2 = GUICtrlCreateLabel("Modell", 1026, 72, 53, 24)
GUICtrlSetFont(-1, 12, 400, 0, "MS Sans Serif")
$Label3 = GUICtrlCreateLabel("Értékesíto", 1026, 104, 79, 24)
GUICtrlSetFont(-1, 12, 400, 0, "MS Sans Serif")
   ...
   For $i=1 to UBound($array)-1
		...
		Next
			...
			If StringLeft($array[$i][13],1) = "N" Then
				GUICtrlCreateListViewItem($array[$i][2] & "|" & StringLeft($array[$i][1],2) & "|" & $array[$i][9] & "|" & $array[$i][6] & "|" & $array[$i][7] & "|" & StringLeft($array[$i][12],1) & "|" & "" & "|" & (StringLeft($array[$i][24],4) & "/" & StringMid($array[$i][24], 5, 2) & "/" & StringMid($array[$i][24], 7, 2))&"|"& $array[$i][33],$List1)
			Else
				GUICtrlCreateListViewItem($array[$i][2] & "|" & StringLeft($array[$i][1],2) & "|" & $array[$i][9] & "|" & $array[$i][6] & "|" & $array[$i][7] & "|" & StringLeft($array[$i][12],1) & "|" & StringLeft($array[$i][13],1) & "|" & (StringLeft($array[$i][24],4) & "/" & StringMid($array[$i][24], 5, 2) & "/" & StringMid($array[$i][24], 7, 2)&"|"& $array[$i][33]),$List1)
			EndIf
    ...
    Switch $nMsg
	    ...
	    Case $kovho
		 Switch GUICtrlRead($kovho)
			Case $GUI_CHECKED
			   GUICtrlSetData($kovho,"Következo hónap")
			   _GUICtrlListView_DeleteAllItems($List1)
			   _GUICtrlListView_DeleteAllItems($List2)
			   lista()
			Case Else
